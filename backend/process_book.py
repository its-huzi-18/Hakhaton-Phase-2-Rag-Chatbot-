...
def create_collection(collection_name: str, vector_size: int = 1024) -> bool:
    ...
    Returns:
        bool: True if collection was created or already exists with correct size
    """
    ...
    try:
        ...
        collection_dict = {col.name: col for col in collections}

        if collection_name in collection_dict:
            # For simplicity, just delete and recreate if it exists
            # since we can't easily check the vector size with this client version
            logger.info(f"Collection '{collection_name}' exists, deleting and recreating with correct size")
