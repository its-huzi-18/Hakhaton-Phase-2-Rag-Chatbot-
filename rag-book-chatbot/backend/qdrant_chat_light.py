...
app = FastAPI(
    title="Book RAG Chatbot with Qdrant - Light Version",
...
def initialize_qdrant_store():
    ...
    # Initialize OpenAI embeddings model (lighter than SentenceTransformers)
    openai_api_key = os.getenv("OPENAI_API_KEY")
    if openai_api_key:
        print("Using OpenAI embeddings")
        embeddings = OpenAIEmbeddings(
            openai_api_key=openai_api_key,
            model="text-embedding-ada-002"  # Using OpenAI embeddings instead of local
        )
    else:
        print("No OpenAI API key found, using local model fallback")
        # For Railway's 4GB limit, we'll use a lighter approach
        # This would require a different embedding strategy if OpenAI key is not provided
        raise ValueError("OpenAI API key is required for this lightweight version")
...
async def root():
    ...
    """
    return {"message": "Book RAG Chatbot with Qdrant (Light Version) is running!", "status": "ok"}
